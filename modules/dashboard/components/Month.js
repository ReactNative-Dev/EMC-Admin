...
import {getOrdersMonth} from "../actions";
...
class Month extends Component {
    ...
    render() {
        const order_per_day = parseFloat(this.state.total / 30).toFixed(2);
        const sale_per_day = parseFloat(this.state.totalSales / 30).toFixed(2);
...
{sale_per_day == 0 && <Text  style={[styles.val, {padding: 5}]}> {sale_per_day}</Text>}
                ...
                <View style={{flexDirection: 'row', position: 'absolute', bottom: 0, borderTopWidth:2, height: 100, width: "100%", borderColor: "#AAB7B8"}}>
                    ...
                    <View style={[styles.bottomText, {borderTopWidth: 0}, {borderBottomWidth: 0}, {borderWidth: 2}]}>
                        <TouchableOpacity onPress={() => this.goToSales({duration: 'month'})}>
...
const mapStateToProps = state => {
    return {
        items: state.dashboard.orders.itemsMonth,
        isLoading: state.dashboard.orders.isLoadingMonth,
        total: state.dashboard.orders.totalMonth,
        totalSales: state.dashboard.orders.totalSalesMonth,
