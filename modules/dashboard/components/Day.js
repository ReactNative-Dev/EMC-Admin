...
class Day extends Component {
    ...
    render() {
        const order_per_hour = parseFloat(this.state.total / 24).toFixed(2);
        const sale_per_hour = parseFloat(this.state.totalSales / 24).toFixed(2);
        const fill_order = (order_per_hour && this.state.total) ? (order_per_hour / this.state.total) * 100 : 0;
        const fill_sale = (sale_per_hour && this.state.totalSales) ? (sale_per_hour / this.state.totalSales) * 100 : 0;
                ...
                <View style={{flexDirection: 'row', width: "100%"}}>
                    <View style={styles.progressBar}>
                        ...
                        <View style={styles.textView}>
                            ...
                            {sale_per_hour > 0 && <Text  style={[styles.val, {padding: 5}]}> {currencySymbol(this.props.items[0].base_currency)} {sale_per_hour}</Text>}
{sale_per_hour == 0 && <Text  style={[styles.val, {padding: 5}]}> {sale_per_hour}</Text>}
                            ...
                            <Text style={[styles.text, {marginRight: 15}]}> HOUR</Text>
                        </View>
                    </View>
                    <View style={styles.progressBar}>
                        <AnimatedGaugeProgress
                        ...
                        <View style={styles.textView}>
                            ...
                            <Text style={[styles.val, {padding: 5}]}>{order_per_hour}</Text>
                            <Image source={require('../../../assets/images/dashboard/percentage_cirlce.png')}
                                   resizeMode={'contain'}
                                   style={{
                                       width: 90,
                ...
                <View style={{
                    ...
                    <View style={[styles.bottomText, {borderTopWidth: 0}, {borderBottomWidth: 0}, {borderWidth: 2}]}>
                        <TouchableOpacity onPress={() => this.goToSales({duration: 'day'})}>
...
const mapStateToProps = state => {
    return {
        items: state.dashboard.orders.itemsDay,
        isLoading: state.dashboard.orders.isLoadingDay,
        total: state.dashboard.orders.totalDay,
        totalSales: state.dashboard.orders.totalSalesDay,
